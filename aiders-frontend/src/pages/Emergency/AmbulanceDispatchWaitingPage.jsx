...
export default function AmbulanceDispatchWaitingPage() {
  ...
  const { selectedAmbulance, transferToHospital, completeTransport, resetHospitalMatching } = useEmergencyStore();
  const { logout } = useAuthStore();

  const [showCancelDialog, setShowCancelDialog] = useState(false);
  const ambulanceStatus = selectedAmbulance?.status?.toLowerCase();
  
  // 상태에 따른 라우팅 로직은 상위 라우터(AmbulanceRouters.jsx)에서 처리하므로,
  // 여기서는 불필요한 useEffect를 삭제하여 충돌을 방지합니다.
