...
const MapDisplay = ({ 
  ...
  const analyzeCoordinateQuality = useCallback((locationData, name = '') => {
    ...
    // 🔥 서울시청 기본 좌표 확인
    if ((Number(lat).toFixed(6) === '37.566826' && Number(lng).toFixed(6) === '126.978657') ||
  ...
  const stableHospitalCoords = useMemo(() => {
    ...
    console.log('[MapDisplay] 🏥 병원 좌표 memoized:', coords, 'from hospital:', hospital.name);
    return coords;
    ...
    const initializeMap = () => {
      try {
        ...
        if (stableAmbulanceCoords) {
          ...
          console.log(`[MapDisplay] 🚑 구급차 위치로 초기화 (${ambulanceQuality.quality}):`, stableAmbulanceCoords);
        } else if (stableHospitalCoords) {
          initialPosition = new window.kakao.maps.LatLng(
            stableHospitalCoords.lat, 
            stableHospitalCoords.lng
          );
          console.log(`[MapDisplay] 🏥 병원 위치로 초기화 (${hospitalQuality.quality}):`, stableHospitalCoords);
        } else {
        ...
        if (stableAmbulanceCoords) {
          ...
          console.log(`[MapDisplay] ✅ 구급차 마커 생성 완료 (${ambulanceQuality.quality})`);
        }
        ...
        if (stableHospitalCoords && hospital) {
          ...
          const hospitalInfoContent = () => {
            let content = `
              <div style="padding:10px;min-width:220px;text-align:center;">
                <strong style="color:#dc2626;">🏥 ${hospital.name || '배정 병원'}</strong>
            `;
          ...
          console.log(`[MapDisplay] ✅ 병원 마커 생성 완료 (${hospitalQuality.quality}):`, hospital.name);
        }
  ...
  // 🔥 구급차 위치 실시간 업데이트 (안정화)
  useEffect(() => {
    if (!mapInstance.current || !stableAmbulanceCoords || !ambulanceMarker.current) {
      return;
    }
    ...
    try {
      ...
      console.log(`[MapDisplay] 🚑 구급차 위치 실시간 업데이트 (${ambulanceQuality.quality}):`, stableAmbulanceCoords);
    } catch (error) {
  ...
  return (
    <div className={`w-full h-full ${isFullScreen ? 'relative' : ''}`}>
      ...
      {distanceInfo && (
        <div className={`${
          ...
          <p className="text-sm font-semibold text-gray-700">
            🚑 ↔ 🏥 실시간 거리: <span className="text-blue-600">{(distanceInfo.distance / 1000).toFixed(2)} km</span>
          </p>
          ...
          <div className="mt-2 flex gap-2 text-xs">
            <span className={`px-2 py-1 rounded ${
              ambulanceQuality.color === 'green' ? 'bg-green-100 text-green-700' :
              ambulanceQuality.color === 'orange' ? 'bg-orange-100 text-orange-700' :
              'bg-red-100 text-red-700'
            }`}>
              🚑 {ambulanceQuality.message}
            </span>
            <span className={`px-2 py-1 rounded ${
              hospitalQuality.color === 'green' ? 'bg-green-100 text-green-700' :
              hospitalQuality.color === 'orange' ? 'bg-orange-100 text-orange-700' :
              'bg-red-100 text-red-700'
            }`}>
              🏥 {hospitalQuality.message}
            </span>
      ...
      {/* 🔥 일반 좌표 품질 경고 */}
      {!hasLocationMismatch && (hospitalQuality.quality === 'default' || hospitalQuality.quality === 'invalid') && (
        <div className={`${
          isFullScreen 
        ...
        <div className="absolute top-4 right-4 flex flex-col gap-2">
          ...
          {ambulanceMarker.current && (
            <button
              ...
              className={`p-2 rounded-lg shadow-lg transition-all ${
                ambulanceQuality.color === 'green' ? 'bg-green-100 hover:bg-green-200 text-green-700' :
                ambulanceQuality.color === 'orange' ? 'bg-orange-100 hover:bg-orange-200 text-orange-700' :
          ...
          {hospitalMarker.current && (
            <button
              ...
              className={`p-2 rounded-lg shadow-lg transition-all ${
                hospitalQuality.color === 'green' ? 'bg-green-100 hover:bg-green-200 text-green-700' :
                hospitalQuality.color === 'orange' ? 'bg-orange-100 hover:bg-orange-200 text-orange-700' :
                'bg-red-100 hover:bg-red-200 text-red-700'
              }`}
      ...
      {import.meta.env.DEV && (
        <div className="absolute top-2 left-2 text-xs text-white bg-black bg-opacity-80 p-2 rounded max-w-xs">
          <div className="font-bold text-yellow-300 mb-1">🔧 지도 디버깅 (안정화)</div>
          
          <div className="space-y-1">
            <div>지도: {isMapReady ? '✅' : '❌'}</div>
            <div>초기화: {initialMapSetupDone.current ? '✅' : '❌'}</div>
            
            <div>구급차: {ambulanceLocation ? '✅' : '❌'} 
              {ambulanceLocation && (
                <span className={`ml-1 px-1 rounded text-xs ${
                  ambulanceQuality.color === 'green' ? 'bg-green-600' :
                  ambulanceQuality.color === 'orange' ? 'bg-orange-600' :
                  'bg-red-600'
                }`}>
            ...
            <div>병원: {hospital ? '✅' : '❌'}
              {hospital && (
                <span className={`ml-1 px-1 rounded text-xs ${
                  hospitalQuality.color === 'green' ? 'bg-green-600' :
                  hospitalQuality.color === 'orange' ? 'bg-orange-600' :
                  'bg-red-600'
                }`}>
            ...
            <div>거리: {distanceInfo ? `${(distanceInfo.distance / 1000).toFixed(2)}km` : '❌'}</div>
            <div>마커: 🚑{ambulanceMarker.current ? '✅' : '❌'} 🏥{hospitalMarker.current ? '✅' : '❌'}</div>
            ...
            {stableAmbulanceCoords && (
              <div className="text-xs border-t border-gray-600 pt-1 mt-1">
                ...
                <div>{stableAmbulanceCoords.lat.toFixed(6)}, {stableAmbulanceCoords.lng.toFixed(6)}</div>
                <div className={ambulanceQuality.color === 'green' ? 'text-green-300' : 
                                ambulanceQuality.color === 'orange' ? 'text-orange-300' : 'text-red-300'}>
                  {ambulanceQuality.message}
                </div>
              </div>
            )}
            ...
            {stableHospitalCoords && hospital && (
              <div className="text-xs border-t border-gray-600 pt-1 mt-1">
                ...
                <div>{stableHospitalCoords.lat.toFixed(6)}, {stableHospitalCoords.lng.toFixed(6)}</div>
                <div className={hospitalQuality.color === 'green' ? 'text-green-300' : 
                                hospitalQuality.color === 'orange' ? 'text-orange-300' : 'text-red-300'}>
                  {hospitalQuality.message}
                </div>
                {hasLocationMismatch && (
